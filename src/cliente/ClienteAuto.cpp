...
string url;
const char* INSTRUCCIONES[] = {"ESTADO", "AYUDA", "PASO", "PUJAR"};
...
int randomT(){
    srand (time(NULL));
    int random = rand() % 2000 + 500;
    return random;
}

int randomD(){
    srand (time(NULL));
    int random = rand() % 1500 + 550;
    return random;
}

void crearMensaje(int n, string& mensaje){
	if(n<=2){
		mensaje = INSTRUCCIONES[n];
	}
	else if(n==3){
		mensaje = INSTRUCCIONES[n];
		mensaje += + " " + to_string(randomD());
	}
}
    ...
    if(mensaje!=MENS_FIN){
		...
		do{
			...
			if(buffer == MENS_FIN){
				...
				// Selecciono instruccion aleatoria
    			this_thread::sleep_for(chrono::milliseconds( randomT() ));
				crearMensaje(randomI(), mensaje);
			}
